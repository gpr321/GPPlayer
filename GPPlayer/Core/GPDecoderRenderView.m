...
static BOOL validateProgram(GLuint prog)
{
    ...
    if (logLength > 0)
    {
        ...
        GPLogVideo(@"Program validate log:\n%s", log);
        free(log);
    }
#endif
    ...
    if (status == GL_FALSE) {
        GPLogVideo(@"Failed to validate program %d", prog);
        return NO;
    }
...
static GLuint compileShader(GLenum type, NSString *shaderString)
{
    ...
    if (shader == 0 || shader == GL_INVALID_ENUM) {
        GPLogVideo(@"Failed to create shader %d", type);
        return 0;
    }
    ...
    if (status == GL_FALSE) {
        ...
        GPLogVideo( @"Failed to compile shader:\n");
        return 0;
    }
...
- (void) setFrame: (GPVideoFrame *) frame;
- (BOOL) prepareRender;
@end
...
- (void) setFrame: (GPVideoFrame *) frame
{
    GPVideoFrameRGB *rgbFrame = (GPVideoFrameRGB *)frame;
...
- (void) setFrame: (GPVideoFrameYUV *) frame
{
    GPVideoFrameYUV *yuvFrame = (GPVideoFrameYUV *)frame;
...
@implementation GPDecoderRenderView {
             ...
             decoder: (GPDecoder *) decoder
{
    ...
    if (self) {
        ...
        if ([decoder setVideoFormat:GPVideoFrameFormatYUV]) {
            
            _renderer = [[KxMovieGLRenderer_YUV alloc] init];
            GPLogVideo(@"OK use YUV GL renderer");
            
        } else {
            
            _renderer = [[KxMovieGLRenderer_RGB alloc] init];
            GPLogVideo(@"OK use RGB GL renderer");
        }
        ...
        if (!_context ||
            ...
            GPLogVideo(@"failed to setup EAGLContext");
            self = nil;
            return nil;
        }
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            GPLogVideo(@"failed to make complete framebuffer object %x", status);
            self = nil;
            return nil;
        }
        ...
        if (GL_NO_ERROR != glError) {
            ...
            GPLogVideo( @"failed to setup GL %x", glError);
            self = nil;
            return nil;
        }
...
- (void)layoutSubviews
{
	...
	if (status != GL_FRAMEBUFFER_COMPLETE) {
        ...
        GPLogVideo(0, @"failed to make complete framebuffer object %x", status);
        
	} else {
        
        GPLogVideo( @"OK setup GL framebuffer %d:%d", _backingWidth, _backingHeight);
    }
...
- (BOOL)loadShaders
{
    ...
    if (status == GL_FALSE) {
		GPLogVideo(0, @"Failed to link program %d", _program);
        goto exit;
    }
...
exit:
    ...
    if (result) {
        ...
        GPLogVideo( @"OK setup GL programm");
...
- (void)render: (GPVideoFrame *) frame
{        
    ...
    if ([_renderer prepareRender]) {
        ...
        {
            GPLogVideo(0, @"Failed to validate program");
            return;
        }
    #endif
